...
# Test the parsing functions in isolation

# Extract just the products function
extract_products_from_sql() {
    ...
    if [ -n "$values_section" ]; then
        while IFS= read -r line; do
            ...
            # Extract specifications (remaining part) and clean it up
            local specifications=$(echo "$clean_line" | sed "s/^'[^']*', *'[^']*', *[0-9.]*, *'[^']*', *[0-9]*, *'\(.*\)'.*$/\1/" | sed 's/}*$//')
