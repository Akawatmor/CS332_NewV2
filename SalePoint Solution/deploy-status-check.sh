...
test_api_functionality() {
    ...
    if command -v jq &> /dev/null; then
        ...
        # If all counts are 0 or unknown, suggest database initialization
        if [[ "$orders_count" == "0" || "$orders_count" == "unknown" ]] && \
           [[ "$products_count" == "0" || "$products_count" == "unknown" ]] && \
           [[ "$customers_count" == "0" || "$customers_count" == "unknown" ]]; then
            print_warning "All tables appear empty. You may need to initialize sample data."
            print_info "Run: ./init-sample-data.sh"
        fi
    else
